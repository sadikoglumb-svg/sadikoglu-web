...
import { Plus, Edit, Trash2, ChevronLeft, Save, X, Upload } from "lucide-react";
import type { TamamlananProje } from "@/lib/projects";

const EMPTY_FORM: Partial<TamamlananProje> = {
  ad: "",
  adres: "",
...
export default function TamamlananProjelerAdmin() {
  const router = useRouter();
  const [projeler, setProjeler] = useState<TamamlananProje[]>([]);
  ...
  const [duzenle, setDuzenle] = useState<TamamlananProje | null>(null);
  const [form, setForm] = useState<Partial<TamamlananProje>>(EMPTY_FORM);
  const [saving, setSaving] = useState(false);
  const [silKonfirm, setSilKonfirm] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/admin/tamamlanan")
      .then((r) => {
  ...
  const acikForm = (proje?: TamamlananProje) => {
  ...
  const kaydet = async () => {
    if (!form.ad || !form.adres) {
      alert("Proje adı ve adres zorunludur.");
      return;
    }
    setSaving(true);
    const method = duzenle ? "PUT" : "POST";
    const res = await fetch("/api/admin/tamamlanan", {
      method,
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-navy text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-serif font-bold text-lg text-white">Tamamlanan Projeler</h1>
        </div>
        <button
          onClick={() => acikForm()}
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="bg-white border border-gray-100 border-t-4 border-t-gold p-8 mb-8 shadow-sm">
            <div className="flex items-center justify-between mb-7">
              <h2 className="font-serif text-xl text-navy font-bold">
                {duzenle ? "Projeyi Düzenle" : "Yeni Tamamlanan Proje Ekle"}
              </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input
                  ...
                  className="input"
                  placeholder="Örn: Barış Apartmanı"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="input"
                  placeholder="Zühtüpaşa Mah., Kadıköy"
                />
              </div>
              <div>
              ...
              <div>
                <label className="label">İskan Tarihi</label>
                <input
                  type="text"
                  value={form.iskanTarihi || ""}
                  onChange={(e) => setForm({ ...form, iskanTarihi: e.target.value })}
                  className="input"
                  placeholder="2024-06"
                />
              </div>
              <div>
                <label className="label">Bağımsız Bölüm Sayısı</label>
                <input
                  type="number"
                  value={form.bagimsizBolum || ""}
                  onChange={(e) => setForm({ ...form, bagimsizBolum: parseInt(e.target.value) })}
                  className="input"
                />
              </div>
              <div>
                <label className="label">İnşaat m²</label>
                <input
                  type="number"
                  value={form.insaatM2 || ""}
                  onChange={(e) => setForm({ ...form, insaatM2: parseInt(e.target.value) })}
                  className="input"
                />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                <label className="label">Fotoğraf Ekle (opsiyonel)</label>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            <p>Henüz tamamlanan proje eklenmedi.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {projeler.map((proje) => (
              <div
                key={proje.id}
                className="bg-white border border-gray-100 p-5 flex items-center justify-between gap-4"
              >
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-navy">{proje.ad}</h3>
                  <p className="text-gray-400 text-sm mt-1">
                    {proje.adres} • {proje.bagimsizBolum} daire • {proje.insaatM2} m² •
                    İskan: {proje.iskanTarihi}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => acikForm(proje)} className="p-2 text-gray-400 hover:text-navy transition-colors">
                    <Edit size={18} />
                  </button>
                  {silKonfirm === proje.id ? (
                    <div className="flex items-center gap-1">
                      <button onClick={() => sil(proje.id)} className="text-xs bg-red-500 text-white px-2 py-1">Evet, sil</button>
                      <button onClick={() => setSilKonfirm(null)} className="text-xs bg-gray-200 px-2 py-1">İptal</button>
                    </div>
                  ) : (
                    <button onClick={() => setSilKonfirm(proje.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors">
                      <Trash2 size={18} />
                    </button>
                  )}
                </div>
